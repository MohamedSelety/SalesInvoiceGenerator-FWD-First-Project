...
import com.udacity.view.InvoFrame;
 ...
 */
 ...
 private InvoFrame frame;
    ...
    public newListener(InvoFrame frame) {
        this.frame = frame;
    }
 @Override
...
case "DeleteLine":
 ...
 case "createInvCancel":
   ...
   private void LoadFile() {
        ...
        if (result == JFileChooser.APPROVE_OPTION) {
            ...
            try{
            FileReader headFr = new FileReader(headerFile);
            BufferedReader headerBr = new BufferedReader (headFr);
            String headLine = null;
            
            while (( headLine = headerBr.readLine()) != null) {
            String[] headParts = headLine.split(",");
            String invoNumStr = headParts[0];
            String invoDateStr = headParts[1];
            String customName = headParts[2];
 
            int invNum = Integer.parseInt(invoNumStr);
            Date invDate = dataf.parse(invoDateStr);
            
            InvoFrame1 inv = new InvoFrame1(invNum, customName, invDate);
 ...
 frame.getInvoicesTable().validate();
    ...
    for(InvoFrame1 inv : frame.getInvoicesArray()) {
...
String headers = "";
        String lines = "";
        for (InvoFrame1 header : frame.getInvoicesArray()) {
...
if (selectedRowIndex >= 0){
    InvoFrame1 row = frame.getInvHeaderTableModel().getInvoicesArray().get(selectedRowIndex);
    frame.getCustNameTF().setText(row.getCustomerName());
    frame.getInvDateTF().setText(dataf.format(row.getInvDate()));
 ...
 private void deleteInvoice() {
        ...
        InvoFrame1 header = frame.getInvHeaderTableModel().getInvoicesArray().get(invIndex);
...
frame.setHeaderDialog(null);
try {
    Date invDate = dataf.parse(invDateStr);
...
double itemPrice = Double.parseDouble(itemPriceStr);
 int headerIndex = frame.getInvoicesTable().getSelectedRow();
        InvoFrame1 invoice = frame.getInvHeaderTableModel().getInvoicesArray().get(headerIndex);
